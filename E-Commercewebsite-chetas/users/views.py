...
def cart(request, idz, typer):
    ...
    if mode == 'add':
        ...
        if cart3:
            cart3.delete()
    cartz = Cart.objects.filter(user=request.user)
    cart2 = []
    catx = Cart.objects.values('product_name')
    cats = {item['product_name'] for item in catx}
    # for cat in cats:
		# product = Cart.objects.filter(product_name=cat)
        # quantity = len(product)
        # cart2.append([product[0], quantity])
